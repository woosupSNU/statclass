...
scatter <- ggplot(examData, aes(Anxiety, Exam))
scatter + geom_point() + 
  geom_smooth() +  #데이터를 잘 설명할 수 있는 smooth line 생성
  labs(x = "Exam Anxiety", y = "Exam Performance %")+ 
  ggtitle("exam anxiety")
...
#Simple scatter with regression line(red)+ CI (confidence interval: 신뢰구간)
...
#Simple scatter with regression line + coloured CI (confidence interval: 신뢰구간)
scatter <- ggplot(examData, aes(Anxiety, Exam))
scatter + geom_point() + 
...
festivalHistogram
#1 outlier cased때문에 그래프가 이상한걸 확인
festivalData %>% 
